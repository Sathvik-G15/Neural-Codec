...
#   - Depthwise-separable convolutions for low FLOPs per stage (~5 GFLOPs)
...
class ProgressiveContextualDecoder(nn.Module):
    ...
    Refinement stages R_1..R_k (feature-space, between part1 and part2):
        features -> features_1 (via R_1) -> features_2 (via R_2) -> features_3 (via R_3)
    ...
    def forward(
        ...
        - During TRAINING (self.training == True): returns all tiers up to target_depth
          for deep supervision loss computation. part2 runs at each tier.
        - During INFERENCE/EVAL (self.training == False): returns ONLY the reconstruction
          at target_depth. part2 runs exactly ONCE at the end.
        ...
        Args:
            ...
            depth:   Number of refinement blocks to apply (1..num_refinement_blocks+1).
                     depth=1 -> base decoder only
                     depth=k -> base + first k-1 refinement blocks
        ...
        """
        if depth is None:
            target_depth = self.num_refinement_blocks + 1
        else:
            target_depth = max(1, min(depth, self.num_refinement_blocks + 1))
        ...
        outputs.append(torch.clamp(base_recon, 0.0, 1.0))

        refined_features = features
        for i in range(target_depth - 1):
            refined_features = self.refinement_blocks[i](refined_features)
            recon = self.part2(torch.cat([refined_features, context], dim=1))
            outputs.append(torch.clamp(recon, 0.0, 1.0))
