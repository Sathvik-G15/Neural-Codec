...
from .progressive_decoder import ProgressiveContextualDecoder
...
class DCVC_net(nn.Module):
    def __init__(self):
        ...
        # Progressive contextual decoder — drop-in replacement for the original
        # contextualDecoder_part1 + contextualDecoder_part2.
        # Tier 1 output is identical to the original DCVC reconstruction.
        # Tiers 2-4 apply lightweight residual refinement blocks on top.
        self.progressive_decoder = ProgressiveContextualDecoder(
    ...
    def compress(self, referframe, input_image):
        ...
        # ----- progressive decode (training uses all depths = full quality) -----
        recon_images = self.progressive_decoder(
            compressed_y_renorm, context, depth=None
        )  # list of (B, 3, H, W)
        ...
        return {"bpp_mv_y": bpp_mv_y,
                ...
                "recon_images": recon_images,   # list of all 4 tier outputs
    ...
    def decompress(self, referframe, mv_y_string, mv_z_string, y_string, z_string, height, width):
        ...
        # Progressive decode — caller controls depth for compute/quality tradeoff
        recon_images = self.progressive_decoder(
            y_hat, context, depth=None
        )  # list of (B, 3, H, W)
        recon_image = recon_images[-1].clamp(0, 1)

        return recon_image   # backward-compatible: return the final-tier output

    def decompress_progressive(self, referframe, mv_y_string, mv_z_string,
                               y_string, z_string, height, width, depth=None):
        """Like decompress() but returns all progressive tier reconstructions.

        Args:
            depth: Number of tiers to run (1-4). None = all.

        Returns:
            List of (B, 3, H, W) tensors, one per tier.
        """
        device = next(self.parameters()).device
        mv_z_size = get_downsampled_shape(height, width, 64)
        mv_z_hat = self.bitEstimator_z_mv.decompress(mv_z_string, mv_z_size)
        ...
        recon_images = self.progressive_decoder(y_hat, context, depth=depth)
    ...
    def load_dict(self, pretrained_dict):
        ...
        for key, weight in pretrained_dict.items():
            ...
            if key[:7] == "module.":
                result_key = key[7:]
            # Map old contextualDecoder keys to new progressive_decoder base
            if result_key.startswith("contextualDecoder_part"):
                result_key = result_key.replace("contextualDecoder_part", "progressive_decoder.part")
            result_dict[result_key] = weight

        self.load_state_dict(result_dict, strict=False)
